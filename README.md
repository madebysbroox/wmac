...
5. For a member who is behind, **알림 이메일 쓰기 (Email Reminder)** opens the computer's own mail program (such as Outlook) with a polite English payment reminder already written — recipient, subject, unpaid months, and total due all filled in. Just click Send. Payments 10 or more days past due (due the 1st of each month) include a one-time late fee of 5% or $5, whichever is greater, with a footnote explaining it. When 2 or more months are behind, the email adds a note that accounts 3+ months behind may go to a collection agency — while urging a phone call to Master Lee at (540) 347-7266 to work things out first. The button explains itself if the member has no email on file or no balance due.
